...
public final class JsonDataToScheduleConverter {
        ...
        if (StringHelper.GetStringCount(value, _searchParameter) >= 1) {
            if (value.contains(_searchParameter)) {
                ...
                for (int index = 0; index < entries.length; index++) {
                    ...
                    _logger.Information(String.format("Schedule entry is %s", entry));

                    String[] data = entry.split("\\};");
                    _logger.Information(String.format("Split data %s has size is %s", data, data.length));
                    Schedule newValue = parseStringToValue(index, data, socketList);
                    if (newValue != null) {
    ...
    private Schedule parseStringToValue(
            int id,
            @NonNull String[] data,
            @NonNull SerializableList<WirelessSocket> socketList) {
        if (data.length == 11) {
            if (data[0].contains("{Name:")
                ...
                if (!isTimer) {
                    Schedule newValue = new Schedule(
                            id,
                            name,
                            "",
                            socket,
