...
import guepardoapps.lucahome.adapter.LibraryListViewAdapter;
...
public class LibraryActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    /**
     * MenuService manages data for magazinDir
     */
    private LibraryService _libraryService;

    /**
     * Adapter for the magazin entries of the listView
     */
    private LibraryListViewAdapter _libraryListViewAdapter;

    /**
     * BroadcastReceiver to receive updates for the library
     */
    private BroadcastReceiver _libraryUpdateReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            _logger.Debug("_libraryUpdateReceiver");
            LibraryService.MagazinDownloadFinishedContent result =
                    (LibraryService.MagazinDownloadFinishedContent) intent.getSerializableExtra(LibraryService.MagazinDownloadFinishedBundle);
            ...
            if (result.Success) {
                if (result.MagazinList != null) {
                    if (result.MagazinList.getSize() > 0) {
                        _libraryListViewAdapter = new LibraryListViewAdapter(_context, result.MagazinList);
                        _listView.setAdapter(_libraryListViewAdapter);
                    ...
                    Snacky.builder()
                            .setActivty(LibraryActivity.this)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        _listView = (ListView) findViewById(R.id.listView_library);
        _progressBar = (ProgressBar) findViewById(R.id.progressBar_library);
        _noDataFallback = (TextView) findViewById(R.id.fallBackTextView_library);

        _searchField = (EditText) findViewById(R.id.search_library);
        _searchField.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence charSequence, int start, int count, int after) {
                SerializableList<MagazinDir> filteredList = _libraryService.FoundMagazins(charSequence.toString());
                _libraryListViewAdapter = new LibraryListViewAdapter(_context, filteredList);
                _listView.setAdapter(_libraryListViewAdapter);
            }
        ...
        SerializableList<MagazinDir> magazinList = _libraryService.GetMagazinList();
        if (magazinList.getSize() > 0) {
            _libraryListViewAdapter = new LibraryListViewAdapter(_context, magazinList);
            _listView.setAdapter(_libraryListViewAdapter);
        ...
        CollapsingToolbarLayout collapsingToolbar = (CollapsingToolbarLayout) findViewById(R.id.collapsing_toolbar_library);
        ...
        DrawerLayout drawer = (DrawerLayout) findViewById(R.id.drawer_layout_library);
    ...
    protected void onResume() {
        ...
        _receiverController.RegisterReceiver(_libraryUpdateReceiver, new String[]{LibraryService.MagazinDownloadFinishedBroadcast});
    }
