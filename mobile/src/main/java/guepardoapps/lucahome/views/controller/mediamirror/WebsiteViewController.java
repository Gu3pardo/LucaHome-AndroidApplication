        ...
        _buttonSendWebsite.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (url.length() < 15 || url.length() > 50) {
                    ...
                    Toasty.error(_context, "Url has invalid length!", Toast.LENGTH_LONG).show();
                    return;
                }
                if (!url.startsWith("http") || !url.startsWith("www")) {
                    _logger.Error("Url has invalid format!");
                    Toasty.error(_context, "Url has invalid format!", Toast.LENGTH_LONG).show();
                    return;
                }

                _mediaMirrorController.SendCommand(
                        _mediaMirrorViewDto.GetMediaServerSelection().GetIp(),
                        MediaServerAction.SHOW_WEBVIEW.toString(),
                        url);
            }
        });
    }
